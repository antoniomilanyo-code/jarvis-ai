...
// When on GitHub Pages, auto-fetch the API endpoint from api-config.json.
  ...
  _apiReady = fetch('./api-config.json?t=' + Date.now())
    ...
    .then(cfg => {
      if (cfg && cfg.api_url) {
        ...
        console.log('[JARVIS] API connected:', API_BASE.substring(0, 60) + '...');
      }
    })
    .catch(() => {
      console.warn('[JARVIS] api-config.json not found — will prompt for API endpoint');
    });
} else {
